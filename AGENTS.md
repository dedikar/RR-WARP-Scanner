...
- Установлена 0.2.1-r41: новая опция «Отбрасывать мёртвые и с потерями»
  (галка в настройках скана). Полностью убирает из результата серверы, чья
  строка помечена torn=1 (DPI рвёт туннель после handshake) ИЛИ loss>0
  (потери в туннеле) — раньше такие строки лишь шли в конец сортировки и не
  выбирались как BEST. Реализация: checkbox scan.js → `drop_bad` в
  saveSettings/scanStart → флаг `-a` в wscan.sh → awk-фильтр при сортировке
