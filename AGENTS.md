...
- Лимиты (0.2.1-r4/r5, потолок воркеров снижен в r16): потолок хостов
  `HOSTS_MAX=4318` (17 подсетей × 254 октета = весь WARP-пул), потолок
  воркеров `JOBS_MAX=70` (IP-адреса воркеров `172.16.7.(2+w)`, `.200` занят
  `wgspeed`). Опытным путём на роутерах: при 197 потоках у части
  роутеров полностью ломается веб-интерфейс, 70 — стабильный компромисс
  времени и нагрузки. КАК МЕНЯТЬ ЛИМИТ (полный чек-лист, все места):
  ...
  5. build-память: физический потолок воркеров = адреса `172.16.7.x`, `.200`
     занят wgspeed → максимум 253, но UI/реальность диктуют 70;
