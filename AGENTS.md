...
## Текущее состояние (2026-09-07)
- Собрана/установлена 0.2.1-r44 (новая версия, тесты ожидаются): шапка аккаунта
  показывает PrivateKey сразу после регистрации + кнопка «Скопировать ключ».
  Бэкенд `accountStatus` уже отдаёт `private_key`; правки только в scan.js
  (`renderAccount`) и `copyToClipboard`/`legacyCopy` (восстановление исходной
  подписи кнопки вместо жёсткого «Скопировать .conf»).
- Установлена 0.2.1-r43: ФИКС «Исключить узел DME» (и любого -e) не работал.
  Причина: в wscan.sh присваивание `key=...` стояло отдельным top-level
  оператором awk; busybox awk (v1.36.1) трактует такую строку как правило с
  неявным print → каждая строка печаталась сырым дублем, и исключённый узел
  (DME) оставался в выводе; строки дублировались, сортировка ломалась.
  Исправление: `printf` (ключ) перенесён ВНУТРЬ блока действия. ВАЖНО на будущее:
  в busybox awk не оставлять `key=...` top-level строкой перед правилом —
  держать присваивание внутри `{ ... }`.
  Заодно: (1) сортировка по RTT реально не работала — ключ `"0 12.3"` с
  `sort -n` сравнивает только ведущий «0», а `sort -k` на этой сборке
  игнорируется; теперь единый числовой ключ `torn?+9000000:0 + int(rtt*100)`,
  `sort -n` даёт порядок по RTT, torn в конце. (2) комбинация -e + -a раньше
  игнорировала -e (ветки были взаимоисключающими) — теперь фильтры compose
  в одном awk-проходе. Проверено на busybox awk роутера в 4 режимах.
  Файл: wscan.sh, блок фильтрации/сортировки после `wait` (см. поля
  `ep colo loc rtt tun_rtt tun_loss torn`).
- Установлена 0.2.1-r42: новая опция «Отображать только стабильные»
  (галка в настройках скана). В результатах остаются только рабочие
  эндпоинты: отбрасываются серверы, чья строка помечена torn=1 (DPI рвёт
  туннель после handshake) ИЛИ loss>0 (потери в туннеле) — раньше такие
  строки лишь шли в конец сортировки и не выбирались как BEST. Реализация:
  checkbox scan.js → `drop_bad` в saveSettings/scanStart → флаг `-a` в
  wscan.sh → awk-фильтр при сортировке
  (`( $7 != "1") && ($6 == "0")`; loss=«?» тоже отбрасывается). Опция
  хранится в /etc/rrws-settings.json как drop_bad. ВАЖНО: фильтр применяется
  на этапе записи результата, поэтому одно и то же значение drop_bad видно
  и в таблице, и в .conf/txt — настройка не «живая», действует на следующий
  скан (нужно пересканировать). getSettings отдаёт drop_bad для чекбокса.
  Подпись галки «Отображать только стабильные» (в черновике r41 было
  «Отбрасывать мёртвые и с потерями» — заменена; r41 на GitHub не финальный
  релиз, ждём тестов r42).
...
- **busybox awk: `key=...` отдельной строкой перед правилом = неявный print.**
  Не оставлять top-level присваивание вне `{ ... }` — busybox awk печатает
  каждую запись сырым дублем (так сломался фильтр -e DME, r43). Всё внутри
  действия: `{ key=...; print key $0 }`. И `sort -n` сравнивает только ведущий
  токен ("0 12.3" → "0"), а `sort -k` на этой сборке — no-op: для числовой
  сортировки по двум критериям собирать ОДИН числовой ключ.
